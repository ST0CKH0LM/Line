...
func handler(request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	client := airtable.NewClient("airtable")
	table := client.GetTable("airtable_DB", "airtable_table")
	sess, err := session.NewSession(&aws.Config{
		Region:      aws.String("ap-southeast-1"),
		Credentials: credentials.NewStaticCredentials("IAM_1", "IAM_2", ""),
	})
	checkerr(err)
	...
	if !validateSignature("LINE_CHANNEL", request.Headers["x-line-signature"], []byte(request.Body)) {
		return events.APIGatewayProxyResponse{
